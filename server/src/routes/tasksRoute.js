...
router.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    const parentGoal = await dbQuery('SELECT user_id FROM goals WHERE id = $1', [goal_id]);
    if (parentGoal.rows.length === 0 || parentGoal.rows[0].user_id !== userId) {
      return res.status(403).json({ msg: 'Forbidden: You do not own the parent goal.' });
    }
    const newTask = await dbQuery('INSERT INTO tasks (title, goal_id, user_id) VALUES ($1, $2, $3) RETURNING *', [title, goal_id, userId]);
